...
На основе WIT-пакета `wassel:foundation` разработан SDK, предоставляющий идиоматичный интерфейс
поверх низкоуровневых привязок. Официальный SDK реализован для Rust и Python; для Go, C\#, C++ и
JavaScript требование совместимости с заявленными языками закрыто эталонными примерами,
демонстрирующими работу с привязками напрямую.

== Понятие SDK

=== Определение SDK в контексте работы

SDK предоставляет доступ к низкоуровневым привязкам к импортируемым интерфейсам, высокоуровневые
...
- Идиоматичность фреймворка: фреймворк должен упрощать разработку и предотвращать частые ошибки
  разработчика и баги.
...
Для упрощения работы с привязками фреймворк предоставляет разработанный слой совместимости с
популярным пакетом `http`, реализуя функции конвертации между типами WASI и пакета. Помимо этого,
фреймворк добавляет типажи `FromRequest` и `IntoResponse`, предлагающие более идиоматичный интерфейс
взаимодействия с SDK.
...
возвращаемого значения реализует `IntoResponse`. В результате макрос генерирует гостевую реализацию
...
использования модуля представлен на рисунке~@listing-http-client-example.
...
принимающие строку с SQL-запросом и итератор параметров. Пример использования клиента представлен на
...
"`execute`". Пример использования клиента представлен на рисунке~@listing-redis-example.
...
#figure(
  caption: [Результат выполнения запроса к плагину на Rust],
  kind: image,
  ```json
...
возвращать объект `Response`. Пример простого обработчика представлен на
...
входящего ответа. Пример HTTP-клиента представлен на рисунке~@listing-python-http-client-example.
...
полем `rows`. Пример использования клиента представлен на рисунке~@listing-python-postgres-example.
...
`RedisArgument_*`. Пример использования клиента представлен на
рисунке~@listing-python-redis-example.

#figure(
  caption: [Пример выполнения запроса к Redis на Python],
  kind: image,
  ```python
  config = redis.ConnectionConfig("redis://localhost:6379")
...
=== Пример плагина <section-python-example>

На рисунке~@listing-python-full-example представлен полный пример плагина, который выполняет
HTTP-запрос к стороннему API и возвращает его ответ. В примере представлены абстрактный класс
...
Примеры фокусируются на обработке HTTP-запросов, однако демонстрируемый подход к генерации привязок
универсален и применим к любым другим WIT-интерфейсам, включая `http-client`, `postgres` и `redis`.
...
`HandleRequest` вместо метода экземпляра. Формирование заголовков и тела ответа представлено на
...
Полная реализация класса-обработчика представлена на рисунке~@listing-csharp-full.
...
аналогично функции `init` в Go, но в идиоматичном для JavaScript стиле. Пример плагина представлен
на рисунке~@listing-js-main.
...
Вспомогательные функции для преобразования типов и формирования заголовков ответа представлены на
...
Запись тела ответа в выходной поток и его финализация представлены на рисунке~@listing-cpp-body.
