...
2. Для каждого вида соединения выполнить:
  ...
  - для мостикового соединения оценить надёжность устройства при отказе элемента 1 и при отказе
    элемента 5

3. Проанализировать полученные результаты:
  - сравнить надёжности последовательного и параллельных соединений
  - оценить влияние отказов различных элементов на надёжность мостикового устройства
...
При таком соединении соединении отказ системы произойдёт при отказе любого из её элементов. Поэтому
в качестве интенсивности отказов системы можно взять сумму отказов её элементов
...
$P(t) = e^(-Lambda_с t)$. Далее приведены результаты вычисления средней наработки до отказа и 90%-ой
наработки до отказа.
...
При параллельном соединении элементов отказ системы произойдёт в случае отказа всех её элементов.
Таким образом, вероятность отказа системы будет равна произведению вероятностей отказа её элементов.
...
Таким образом, вероятность безотказной работы при экспоненциальном распределении отказов можно
...
Среднюю наработку до отказа можно вычислить, проинтегрировав ВБР. Результаты интегрирования
приведены далее:
...
Далее приведены результаты вычислений средней наработки до отказа и 90%-ой наработки до отказа.
...
Смешанное соединение можно представить как последовательное, объединив последовательные участки
схемы в целые логические элементы.
...
В то же время, гамма-процентную наработку до отказа не получится вывести из уравнения ВБР, поэтому
далее она будет вычислена численным методом.
...
Можно упростить схему, заменив последовательное соединение элементов 1 и 2 на элемент 6
($lambda_6 = lambda_1 + lambda_2$) и последовательное соединение элементов 3 и 4 на элемент 7
($lambda_7 = lambda_3 + lambda_4$). Результат такой замены показан на рисунке #ref(
  label("img-bridge-simplified"),
  supplement: none,
).
...
По методу минимальных путей можно свести схему мостикового соединения к схеме параллельного
соединения. Далее перечислены все параллельные ветви схемы:
+ Путь: $6 -> 7$. (2 раза) \
...
После подстановки $lambda_6 = lambda_1 + lambda_2$ и $lambda_7 = lambda_3 + lambda_4$ получится:
...
Численным методом можно получить значения показателей надёжности (таблица #ref(
  label("table-bridged"),
  supplement: none,
)). В данной модели элемент 5 имеет интенсивность отказов на порядок выше, чем у всех остальных
элементов, поэтому его отказ не оказывает значительного влияния на надёжность системы. В то же время
отказ элемента 1 значительно ухудшает показатели надёжности.
...
#let parallel-item(a, b, c) = [
  $#a -> #b -> #c$, ВБР: $P_i (t) = e^(-(lambda_#a + lambda_#b + lambda_#c) t)$
]

@img-3-by-5 показывает схему соединения 3 на 5. ВБР такой схемы можно найти методом минимальных
путей, сведя к схеме параллельного соединения. Далее приведены все параллельные ветви такого
соединения:
+ #parallel-item(1, 2, 3)
+ #parallel-item(1, 2, 4)
+ #parallel-item(1, 2, 5)
