...
WIT (Wasm Interface Types) --- это IDL (язык описания интерфейса), позволяющий определить контракт
взаимодействия между хостом (средой выполнения) и гостем (плагином). Определения WIT компилируются
инструментом `wit-bindgen` в привязки для конкретных языков программирования, обеспечивая
типобезопасное взаимодействие между компонентами. В системе определяется WIT-пакет
"`wassel:foundation`", на основе которого:
...
- *postgres.wit* --- определения интерфейса исходящих запросов к СУБД Postgres;
...
Postgres, Redis). Мир `http-plugin` наследует `platform` и дополнительно экспортирует интерфейс
`http-handler`, который плагин обязан реализовать.
