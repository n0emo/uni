...
Обрати внимание, что вспомогательная функция `toField` вынесена внутрь `sendResponse` --- при
желании её можно поднять на уровень пакета, если она потребуется в нескольких местах.
