...
use wgpu::{util::{BufferInitDescriptor, DeviceExt as _}, BlendState, ColorTargetState, ColorWrites, CommandEncoderDescriptor, FragmentState, Operations, PipelineCompilationOptions, PrimitiveState, RenderPassColorAttachment, RenderPassDescriptor, RenderPipelineDescriptor, VertexAttribute, VertexBufferLayout, VertexState, VertexStepMode};
...
impl framework::Application for Application {
    ...
    const DESCRIPTION: &str = "Курсовой проект - реализация трёхмерной сцены с помощью Ray marching";
    ...
    fn render(
        ...
        _dt: f32,
        WgpuContext { instance: _, adapter: _, device, queue }: &WgpuContext
    ) {
