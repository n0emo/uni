...
Рассчитать показатели надёжности для исходной и доработанной системы. Построить графики зависимостей
...
Можно заметить, что средняя наработка до отказа систему ($approx 370 hour$) значительно меньше
средней наработки каждого отдельного элемента. Это говорит о том, что система менее надёжна её
...
Рассчитать гамма-процентное время наработки системы до отказа при различных значениях при различных
значениях. Результаты занести в таблицу 3. Построить график зависимости $T_gamma (gamma)$.
...
Рассчитать среднее время восстановления системы $overline(T)_("вc")$. Сделать выводы о соотношении
$overline(T)_("вc")$ с $t_(в i)$.
...
Рассчитать среднее время восстановления системы после повышения надежности (доработок) некоторых
элементов (второй столбец таблицы 1). Сделать выводы о том, какие ПН системы улучшились, а какие
ухудшились, и подкрепить их расчетами.
...
Можно сделать вывод, что система после даработки стала значительно надёжнее. Несмотря на то, что
время восстановления немного увеличилось, средняя наработка до отказа увеличилась на #{
  int(526 / 370 * 100 - 100)
}%.

== Задание 7

Рассчитать коэффициент готовности системы в конце межрегламентного периода и средний коэффициент
готовности системы при условии, что ее состояние не контролируется для различных вариантов
регламентных работ (таблица 2). Построить график изменения коэффициента готовности от времени (в
течение цикла).

=== Решение

Из предыдущего задания, коэффициент готовности для экспоненциального распределения
...
межрегламентного периода и средний коэффициент готовности доработанной системы при условии, что ее
состояние не контролируется для различных вариантов регламентных работ (таблица 2). Построить график
изменения среднего коэффициента готовности от продолжительности межрегламентного периода.
...
непрерывно контролируется в межрегламентный период для различных вариантов регламентных работ
(таблица 2). Сравнить результаты, сделать выводы. Построить график изменения среднего коэффициента
готовности от продолжительности межрегламентного периода.
...
#figure(
  ...
  caption: [КГ в конце периода обслуживания и средний КГ непрерывно контролируемой исходной
    системы],
)

== Задание 10

Рассчитать коэффициент готовности (средний и стационарный) доработанной системы при условии, что
система непрерывно контролируется в межрегламентный период для различных вариантов регламентных
работ (таблица 2). Сравнить результаты, сделать выводы.
...
#figure(
  ...
  caption: [КГ в конце периода обслуживания и средний КГ непрерывно контролируемой доработанной
    системы],
)

== Задание 11

Рассчитать коэффициент оперативной готовности исходной и доработанной системы, не контролируемой в
межрегламентный период, за заданное время (четвертый столбец таблицы 1).
...
На таблице далее приведены значения КОГ для различных промежутков времени исходной и доработанной
системы
...
Рассчитать коэффициент технического использования системы при различных значениях продолжительности
регламентных работ.
...
Результаты вычислений для исходной и доработанной системы при различных режимах регламентных работ
приведена на таблице
