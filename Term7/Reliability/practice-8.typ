...
Самостоятельно решить задачу оценивания ошибок перваого и второго рода при планировании выборочного
контроля.
...
#let diagbox(text_left, text_right, start: "top", ..kwargs) = {
  let content = context {
    ...
    if start == "bottom" {
      ...
      panic(
        "Unhandled value '" + start + "' for start parameter, expected 'top' or 'bottom'",
      )
    }
  }
...
Результаты вычислений вероятностей возможных исходов испытаний по закону биномиального распределения
для разных $s$ и $k$ представлены на таблице #ref(label("table-binomial"), supplement: none).
...
Вероятности допустить ошибки первого и второго рода показаны на таблице #ref(
  label("table-errors"),
  supplement: none,
).
...
К значениям ошибок $alpha и beta$ обычно применяют жесткие требования: их допустимые значения не
должны превышать $0.1$. Полученные в примере значения ошибок не подходят под требования. Это
свидетельствует о некачественности выборки.
