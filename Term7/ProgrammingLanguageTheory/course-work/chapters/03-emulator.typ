...
в случае отсутствия библиотеки в системе (см. рисунок @img-cmake-raylib)
...
содержащий "устройства" памяти (см. рисунок @img-mem-model).
...
объявлена структура (см. рисунок @img-cpu-model), содержащая в себе поля,
...
+ создание окна размером 920x640 пикселов;
+ установка целевой частоты кадров 60 герц;
+ создание объекта приложение;
+ организация цикла приложения;
+ закрытие окна.
...
Результат запуска приложения показан на рисунке @img-gui. Можно заметить, что
помимо виртуального дисплея на окне отображается информация о регистрах
микропроцессора и служебных значениях, а также набор доступных горячих клавиш.

#figure(
  image("/assets/gui.png", width: 80%),
